...
if [ ! "$OUTPUT" == "build" ] && [ ! "$OUTPUT" == "install" ]; then
	...
	echo "With SputnikVM, running evm $OUTPUT ..."
fi
...
if [ "$OUTPUT" == "install" ]; then
	CGO_LDFLAGS=$LDFLAGS go install -ldflags '-X main.Version='$(git describe --tags) -tags="sputnikvm netgo" ./cmd/evm
elif [ "$OUTPUT" == "build" ]; then
	mkdir -p "$geth_bindir"
	CGO_LDFLAGS=$LDFLAGS go build -ldflags '-X main.Version='$(git describe --tags) -o $geth_bindir/evm -tags="sputnikvm netgo" ./cmd/evm
fi
